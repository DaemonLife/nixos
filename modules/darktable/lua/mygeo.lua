...
local function open_location_in_gnome_maps()
  ...
  for jj, image in ipairs(sel_images) do
    if
      (image.longitude and image.latitude)
      and (image.longitude ~= 0 and image.latitude ~= 90) -- Sometimes the north-pole but most likely just wrong data
    then
...
local function reverse_geocode()
  ...
  startCommand = string.format(
    "curl --silent \"https://api.mapbox.com/geocoding/v5/mapbox.places/%s,%s.json?types=%s&access_token=%s\" | jq '.features | .[0] | '.text''",
    lon1,
    lat1,
    types,
    tokan
...
local function get_distance(lat1, lon1, ele1, lat2, lon2, ele2)
  ...
  local a = math.sin(dLat / 2) * math.sin(dLat / 2)
    + math.cos(math.rad(lat1)) * math.cos(math.rad(lat2)) * math.sin(dLon / 2) * math.sin(dLon / 2)
...
local function altitude_profile()
  ...
  for jj, image in ipairs(sel_images) do
    if
      (not isnan(image.longitude) and not isnan(image.latitude) and not isnan(image.elevation) and image.elevation)
      and (image.longitude ~= 0 and image.latitude ~= 90) -- Sometimes the north-pole but most likely just wrong data
    then
      i = i + 1
...
local function restart()
  dt.register_event(
    ...
    "shortcut",
    print_calc_distance,
    _("calculate the distance from latitude and longitude in km")
  )
  dt.register_event("geoToolbox", "mouse-over-image-changed", toolbox_calc_distance)

  dt.register_event("geoToolbox_wg", "shortcut", select_with_gps, _("select all images with GPS information"))
...
if dt.gui.current_view().id == "lighttable" then
  ...
  if not gT.event_registered then
    dt.register_event("geoToolbox", "view-changed", function(event, old_view, new_view)
